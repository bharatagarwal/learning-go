...
def _append_parent_sections(lines: list[str], payload: dict[str, Any]) -> None:
    ...
    for parent in parent_items:
        ...
        lines.append(f"Cosine score: {float(parent['score']):.6f}")
        lines.append("Text:")
...
def _append_child_chunks(lines: list[str], payload: dict[str, Any]) -> None:
    chunk_items = cast(list[dict[str, Any]], payload.get("matches") or [])
    if not chunk_items:
        return
    ...
    for match in chunk_items:
        ...
        lines.append(f"Cosine score: {float(match['score']):.6f}")
        lines.append("Text:")
